"""Experiment 04 trainer — Model Capacity Ablation.

Research Question: Can increasing model capacity (while remaining under the 2M parameter limit) improve BPB?

CHANGE LOG (relative to starter/train.py):
  - Modified model configuration after instantiation to increase `n_embd`.
  - `cfg.n_embd = 192` (increased from 160).
  - Keeps `n_head = 4` (192/4 = 48 dim per head, which is valid).
  - Target params: 1,902,720 (fits between 1.85M and 1.95M limit).

Everything else is byte-for-byte identical to starter/train.py.
...
def main():
    ...
    # Experiment 04 change: Scale embedding dimension to utilize more budget
    cfg.n_embd = 192
    # Resulting params: 1,902,720
