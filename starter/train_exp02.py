"""Experiment 02 trainer — LR Scheduler Ablation.

Research Question: Can a learning rate schedule (Linear Warmup + Cosine Decay) improve BPB while keeping the optimizer and model unchanged?

CHANGE LOG (relative to starter/train.py):
  - Added `math` to imports for cosine calculation.
  - Added `get_lr(step)` function to compute 100-step linear warmup + cosine decay.
  - Inside training loop, updated optimizer learning rate per step.
...
def main():
    ...
    # Experiment 02 change: Learning Rate Schedule
    def get_lr(step):
        warmup_steps = 100
        if step <= warmup_steps:
            return args.lr * step / warmup_steps
        decay_ratio = (step - warmup_steps) / (args.steps - warmup_steps)
